 ...
 *   auth         -> challenge auth using on-chain DID resolution (legacy mode)
 *   vc           -> VC issuance + verification (legacy mode)
 *   presentation -> END-TO-END PRESENTATION LATENCY (this folder's headline experiment)
...
import { runPresentationLatencyExperiment } from "./experimentPresentationLatency.js";
...
async function main(): Promise<void> {
  ...
  switch (cmd) {
    ...
    case "vc":
      ...
      break;
    case "presentation":
      await runPresentationLatencyExperiment();
      break;
    case "all":
      await runEstablishmentExperiment();
      await runChallengeAuthExperiment();
