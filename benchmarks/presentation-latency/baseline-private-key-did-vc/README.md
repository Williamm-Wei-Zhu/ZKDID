# Private-Key DID/VC — End-to-End Off-Chain Identity/Credential Presentation Latency

> **Variant of the zkEHR comparative-presentation-latency series.** This
> measures the Private-key DID/VC data point. Sibling experiments
> (`oidc-only-presentation-latency`, ZK-based VC, zkDIDProof) measure the
> same end-to-end flow against alternative presentation schemes.
>
> **Flow measured here (timed in a single per-run total):**
>   1. Patient obtains VC from issuer (issuer signs a fresh VC for subject DID)
...
>   6. Hospital resolves DID on Sui Devnet (RPC fetch of `DIDObject`)
>   7. Hospital verifies DID signature and VC
>   8. Hospital creates local patient session
>
> Headline command: `npm run experiment:presentation` (CSV →
> `results/private_key_did_vc_presentation_latency.csv`).
>
> Forked from `private-key-did-vc-sui-devnet/` on 2026-05-02. Adds one new
> file (`src/experimentPresentationLatency.ts`) that combines challenge auth
> with VC presentation in a single timed pipeline. Reuses the existing pool
> of established DIDs in `data/dids.json` and the deployed Move package on
> Sui Devnet — no new on-chain setup needed.
