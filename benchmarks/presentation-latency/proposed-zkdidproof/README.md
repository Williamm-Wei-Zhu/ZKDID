# zkDIDProof — End-to-End Off-Chain Identity/Credential Presentation Latency

> **Variant of the zkEHR comparative-presentation-latency series — this
> paper's solution.** Sibling experiments (`oidc-only-presentation-latency`,
> `private-key-did-vc-presentation-latency`, `zk-vc-presentation-latency`)
> measure the same end-to-end flow against alternative schemes for direct
> comparison.
>
> **Flow measured here (timed in a single per-run total):**
>   1. Hospital issues challenge (random field element)
>   2. Nonce generation: `nonce_jwt = Poseidon(r, T_exp, chal)` (binds challenge into the OIDC nonce)
>   3. **Google JWT request** (silent SSO via primed Chromium profile, with our Poseidon-bound nonce)
>   4. Patient generates challenge-bound ZK proof over OIDC-derived DID (Groth16 / bn128)
>   5. **Hospital verifies the proof LOCALLY — no blockchain access**
>   6. Hospital creates local patient session
>
> **Why this matters:** unlike the `private-key-did-vc-*` and `zk-vc-*`
> variants, the hospital does not perform a Sui RPC fetch or any on-chain
> lookup at presentation time. The verification path is `vk + proof + public
> signals`, all in-process. The DID is derived in-circuit from
> `Poseidon(sub, iss, salt)`.
>
> Headline command: `npm run experiment:zkdidproof` (CSV →
> `results/zkdidproof_presentation_latency.csv`).
>
> Built by combining `oidc-only-presentation-latency/` (for the Playwright +
> Google OIDC silent-SSO infrastructure) with `zkdid-circuit/` (for the
> Groth16 algorithm). Adds `snarkjs` + `circomlibjs` as deps and one new
> orchestrator file `src/experimentZkdidProof.ts`. The compiled circuit
> artifacts (`build/zkdid.generated_js/zkdid.generated.wasm`,
> `build/zkdid_final.zkey`, `build/verification_key.json`) come from the
> sibling `zkdid-circuit/` project — copy them into `./build/` before
