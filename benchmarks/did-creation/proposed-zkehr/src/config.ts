...
import { parseInstitutionsFromEnv, type InstitutionConfig } from "./saltService.js";
loadDotenv();
...
export interface AppConfig {
  ...
  /** Multi-authority salt institutions (zkEHR's defining feature). */
  saltInstitutions: InstitutionConfig[];
  runs: number;
  warmupRuns: number;
  outputDir: string;
  browser: {
...
const saltInstitutions = parseInstitutionsFromEnv(required("SALT_INSTITUTIONS"));
if (saltInstitutions.length === 0) {
  throw new Error("SALT_INSTITUTIONS must list at least one institution URL.");
}
