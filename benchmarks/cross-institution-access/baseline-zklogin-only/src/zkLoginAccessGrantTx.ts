/**
 * Sponsored zkLogin transaction that calls
 *   access_grant::access_grant::create_access_grant(...).
 *
 * Mirrors the existing `zkLoginTx.ts` but targets the AccessGrant Move
 * module instead of did_registry. The patient's zkLogin Sui address is the
 * sender (and becomes the on-chain owner of the new AccessGrant); the
 * sponsor (cfg.privateKey) pays gas.
 */
...
export async function buildSponsoredCreateAccessGrantTx(
  ...
  tx.moveCall({
    ...
    arguments: [
      tx.pure(VECTOR_U8.serialize(Array.from(args.patientId))),
      tx.pure(VECTOR_U8.serialize(Array.from(args.dataHolderHospitalId))),
      tx.pure(VECTOR_U8.serialize(Array.from(args.granteeHospitalId))),
...
export interface ExecAccessGrantResult {
  ...
  submit_ms: number;
  extract_ms: number;
  /** AccessGrant object id (the only AddressOwner-owned creation in the PTB). */
  objectId: string;
  nextSponsorGasCoin: GasCoinRef;
}

/**
 * Sign the tx with both the ephemeral key (wrapped in zkLogin signature
 * carrying the proof) and the sponsor key, then submit via
 * `WaitForEffectsCert`. Identical submission semantics to the existing
 * baseline so cross-baseline numbers stay comparable.
 */
