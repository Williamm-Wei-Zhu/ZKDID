/**
 * Tiny JSON-backed grant/request store. Used so the verification experiment
 * can load grants the creation experiment recorded. Not encrypted — local
 * academic artifact only.
 */
...
export async function loadGrants(path: string): Promise<StoredAccessGrant[]> {
  return readJsonArray<StoredAccessGrant>(path);
}
